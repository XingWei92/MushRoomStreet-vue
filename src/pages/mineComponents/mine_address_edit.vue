	...
	<div>
		<div class="mine_address_header">
			<div>
				...
				<div class="head_title">修改收货地址</div>
			...
			<div class="mine_address_addinfo">
				<div class="reveice_name">
					<label>收货人</label>
					<input type="text" id="name" :value="edit_info.name">
					<div></div>
				</div>
				<div class="phone_number">
					<label>手机号</label>
					<input type="text" id="phone" :value="edit_info.phone">
					<div></div>
				</div>
				<div class="native_place">
					<div class="province">
						<label>省份</label>
						<select  v-model="id" :value="edit_info.id" id="province" >
					...
					<div class="city">
						<label>城市</label>
						<select id="city" :value="edit_info.city" v-model="id1">
					...
					<div class="area">
						<label>地区</label>
						<select id="area" :value="edit_info.area" v-model="id2" >
			...
			<div class="street_info">
				<label>详细地址</label>
				<textarea id="detailed" :value="edit_info.detailed"></textarea>
				<div>0/100</div>
			</div>
			<div class="zip_code">
				<label>邮编</label>
				<input type="text" id="code" :value="edit_info.code">
				<div></div>
			</div>
		</div>
	...
	export default{
		...
		methods:{
			...
			save () {
				...
				if ($("#name")[0].value == ""||reg.test($("#name")[0].value)==false) {
					...
					console.log(this.address_info.name);
		...
		created () {
			...
			this.$http.get("https://s10.mogucdn.com/new1/v1/bmisc/40d4eee5b20031155980b0dcc8dfd695/199706005307.json").then(data => {
				...
				this.edit_info = JSON.parse(sessionStorage.getItem('address_info') )[JSON.parse(sessionStorage.getItem('edit_index') )];
				for (var k = 0;k < this.provinceData.length;k ++) {
					if(this.edit_info.province == this.provinceData[k].name){
						this.edit_info.id = this.provinceData[k].id;
					}
				}
			})
